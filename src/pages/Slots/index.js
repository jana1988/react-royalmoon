...
import { Search, RtpPercent } from "@/components/Ui/Icon";
import {
  isMobile,
  sortFirstString,
  gameApiNameReplace,
...
import { gameTypes, nav, rtpCategoryCodes } from "./constant";
...
export default () => {
  ...
  const fetchGames = async () => {
    const config = {};
    if (featured && gameTypeId !== 15) config.featured = featured;
  ...
  const categoryCodeLabel = rtpCategoryCodes.find(
  ...
  const handleCategoryCode = (value) => {
    init();
    ...
    games.map((item, index) => (
      <Card
        ...
        className={`${styles.card} ${
          isRtpDn(
            item[
              rtpCategoryCodes.filter((item) => item.id === categoryCode)[0]
                ?.rtp
            ],
            item.rtp,
          )
        ...
        visibleRtp={current.index === index && visibleRtp}>
        {navIndex === 2 && (
          <CardRtp
            rtp={item.rtp}
            timeRtp={
          ...
          {nav.map((item, index) => (
            <div
              className={`${styles[`text_${intl.locale}`]} ${
                navIndex === item.id ? styles.active : ""
              }`}
              key={index}
                ...
                text={
                  <span
                    className={`${styles.select_text}  ${
                      categoryCodeLabel.dn ? styles.dn : ""
                    }`}>
                    {categoryCodeLabel.title}
                    <RtpPercent />
                  </span>
                }
                buttonClassName={styles.selectButton}>
                <RadioGroup onChange={handleCategoryCode}>
                  {rtpCategoryCodes.map((item, index) => (
                    <Radio
                      key={index}
                      value={item.id}
                      checked={categoryCode === item.id}
        ...
        {gameTypes.map((item) => (
          <Button
            ...
            className={`${styles.label} ${
              item.id === gameTypeId || item.id === featured
                ? styles.active
                : ""
            }`}
            onClick={() => {
